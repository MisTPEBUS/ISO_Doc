...
export function UsersPage() {
  ...
  const hasFilters =
    keyword.length > 0 || selectedDeptId.length > 0 || includeInactive;
    ...
    createUser.mutate(
      {
        ...
        passwordConfirmation: parsed.data.password
  ...
  function confirmDelete() {
    ...
    deleteUser.mutate(deleteTarget.id, {
      ...
      onError: (error) =>
        setDeleteError(errorMessage(error, "無法停用使用者。")),
    });
  }
  ...
  function confirmResetPassword() {
    ...
    resetPassword.mutate(resetTarget.id, {
      ...
      onError: (error) =>
        setResetError(errorMessage(error, "無法重設使用者密碼。")),
    });
  }
  ...
  const columns: ReadonlyArray<TableColumn<UserResponse>> = [
    ...
    },
    {
      ...
      render: (user) => {
        const canManage =
          isSystemAdmin || user.role !== MANAGED_USER_ROLE.SystemAdmin;
        if (!canManage)
          return <span className="text-meta text-ink-muted">無可用操作</span>;
    ...
    <section>
        ...
        >
          ...
          >
            <Input
              ...
              className={
                editingUser ? "bg-surface-header text-ink-muted" : undefined
              }
              error={fieldErrors.empno !== undefined}
              aria-describedby={
                fieldErrors.empno ? "user-empno-error" : undefined
              }
          ...
          >
            <Input
              ...
              aria-describedby={
                fieldErrors.email ? "user-email-error" : "user-email-hint"
              }
          ...
          >
            <Select
              ...
              aria-describedby={
                fieldErrors.deptId ? "user-dept-error" : undefined
              }
      ...
      >
        ...
        <p className="text-cell text-ink">
          確定要停用「
          <strong className="font-semibold">{deleteTarget?.name}</strong>」嗎？
        </p>
      </Modal>
