...
function toApiError(error: AxiosError): ApiError {
  ...
  const status = typeof problem.status === 'number'
    ? problem.status
    : (response?.status ?? 0)
  ...
  const instance = typeof problem.instance === 'string' ? problem.instance : undefined
  const problemDetails: ProblemDetails = { status, title, detail, type, instance }
