...
- 附件從屬於特定 `document_version_id`，不可跨版本共用。附件為選配（版本可 0 附件），可先建立中繼資料（`attachment_no` + `name`、檔案欄位為 NULL）後補檔；未補檔的附件不可下載、備份略過。
- 附件上傳 API 單次可收多個附件檔，批次採全有全無：任一檔案驗證失敗整批 400，不寫入任何檔案、不建立任何列。
